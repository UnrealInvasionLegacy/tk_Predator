class ElitePredatorProj extends Projectile 
	config(tk_Monsters);
...
simulated function PostBeginPlay()
{
	...
	{
		SmokeTrail = Spawn(class'ElitePredTrail',self,,Location - 40 * Dir, Rotation);
...
simulated function PostNetBeginPlay()
{
	...
	if ( FlockIndex != 0 )
	{
	    ...
	    if ( Flock[1] == None )
	    {
			ForEach DynamicActors(class'ElitePredatorProj',R)
...
simulated function Explode(vector HitLocation,vector HitNormal)
{
	...
	{
	    Spawn(class'ElitePredatorExplodeEffect',,, Location);/////////////////explosion effect << here it is!
		if ( !Level.bDropDetail && (Level.DetailMode != DM_Low) )
			Spawn(class'ElitePredatorExplodeEffect',,, Location);///////////////////////^^explode effect << and again here!
	}
...
defaultproperties
{
     ...
     LifeSpan=10.000000
     Texture=Texture'tk_Predator.Predator.Predimagered'
     DrawScale=0.300000
     Skins(0)=Texture'tk_Predator.Predator.Predimagered'
     AmbientGlow=96
